...
import { X, CheckCircle, AlertTriangle, ShieldAlert, TrendingUp, TrendingDown, Ruler, RotateCcw, Bell, Briefcase, Save, Clock, Loader2, Layers, Target, Activity, Zap, Crosshair, ArrowUpRight, Database, Check, Calculator, DollarSign, Percent, Info } from 'lucide-react';
...
interface Props {
  stock: StockResult;
  onClose: () => void;
  onUpdate?: (updatedStock: StockResult) => void;
  isWatched: boolean;
  onWatchlistToggle: (ticker: string, stockData?: StockResult) => void;
}
...
const DetailView: React.FC<Props> = ({ stock, onClose, onUpdate, isWatched, onWatchlistToggle }) => {
  ...
  const [calcAccountSize, setCalcAccountSize] = useState<number>(100000);
  const [calcRiskPct, setCalcRiskPct] = useState<number>(1.0);

  useEffect(() => {
      const timer = requestAnimationFrame(() => { setIsChartReady(true); });
      
      // Initialize calculator with dynamic Portfolio Total Account Value logic
      const user = userService.getCurrentUser();
      const lastTotalStr = localStorage.getItem(TOTAL_VALUE_KEY);
      
      if (lastTotalStr) {
          // 1. Highest priority: The actual Total Account Value from Holdings screen (if calculated)
          setCalcAccountSize(parseFloat(lastTotalStr));
      } else if (user) {
          // 2. Fallback: Calculate "Estimated Value" (Cash + sum of holdings at cost)
          const cash = user.portfolioStats?.cashBalance || 0;
          const costBasis = user.positions.reduce((sum, p) => sum + (p.avgEntryPrice * p.quantity), 0);
          const estimate = cash + costBasis;
          
          if (estimate > 0) {
              setCalcAccountSize(estimate);
          } else if (user.portfolioStats?.equityInvested) {
              // 3. Last fallback: Principal invested
              setCalcAccountSize(user.portfolioStats.equityInvested);
          }
      }
  ...
  const handleWatchlistClick = () => {
      onWatchlistToggle(displayStock.ticker, displayStock);
  };
  ...
  useEffect(() => {
      const loadData = async () => {
          ...
          const isStale = displayStock.analysisTimestamp ? (Date.now() - displayStock.analysisTimestamp > 600000) : true;

          if (needsHistory || needsProfile || needsMarketCap || isStale) {
  ...
  useEffect(() => {
    ...
    if (pos) {
        ...
        // Default entry price for calculator to current price
        setEntryPrice(displayStock.currentPrice.toFixed(2));
    }
  }, [displayStock]);
  ...
  const calcData = useMemo(() => {
      ...
      const score = displayStock.totalScore;
      const price = displayStock.currentPrice;
      
      // Step A: Max Risk in Dollars
      const maxRiskDollars = calcAccountSize * (calcRiskPct / 100);
      
      // Step B: Stop Distance (2.2 ATR)
      const stopDistance = 2.2 * atr;
      const suggestedStopPrice = price - stopDistance;
      
      // Step C: Base Shares
      const baseShares = stopDistance > 0 ? maxRiskDollars / stopDistance : 0;
      
      // Step D: Alpha Factor
      let alphaFactor = 0;
      let factorLabel = "Do Not Trade";
      let factorColor = "text-red-400";
      
      if (score >= 8.0) {
          alphaFactor = 1.0;
          factorLabel = "Elite Prime (100%)";
          factorColor = "text-green-400";
      } else if (score >= 7.0) {
          alphaFactor = 0.7;
          factorLabel = "Trend Entry (70%)";
      ...
      // Final Calc
      const finalShares = Math.floor(baseShares * alphaFactor);
      const capitalRequired = finalShares * price;
  ...
  }, [calcAccountSize, calcRiskPct, displayStock]);

  const applyCalculatedPosition = () => {
      setHasPosition(true);
      setShares(calcData.finalShares.toString());
      setEntryPrice(displayStock.currentPrice.toFixed(2));
  };

  const splitStats = useMemo(() => {
      if (!displayStock.backtest || !displayStock.backtest.trades) return { primeWins: 0, primeTotal: 0, trendWins: 0, trendTotal: 0 };
      const primeTrades = displayStock.backtest.trades.filter(t => t.signalType === 'PRIME');
      const trendTrades = displayStock.backtest.trades.filter(t => t.signalType === 'TREND');
      const primeWins = primeTrades.filter(t => t.pnlPercent > 0).length;
      const trendWins = trendTrades.filter(t => t.pnlPercent > 0).length;
      return {
          primeWins,
          primeTotal: primeTrades.length,
          primeRate: primeTrades.length > 0 ? (primeWins / primeTrades.length) * 100 : 0,
          trendWins,
          trendTotal: trendTrades.length,
          trendRate: trendTrades.length > 0 ? (trendWins / trendTrades.length) * 100 : 0
      };
  }, [displayStock.backtest]);
  ...
  const BacktestTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const percentChange = ((equity - 10000) / 10000) * 100;
      
      const status = data.status === 'Long' ? "LONG (In Market)" : "CASH (Sidelines)";
      const statusColor = data.status === 'Long' ? "text-green-400" : "text-gray-400";
            ...
            {data.isEntry && (
              <div className="mt-2 pt-2 border-t border-gray-800">
                <div className={`${data.isPrime ? 'text-yellow-400' : 'text-blue-400'} font-bold text-[10px] uppercase flex items-center gap-1`}>
                  <Target size={10} /> {data.entryReason || 'Signal Executed'}
                </div>
              </div>
            )}

            {!data.isEntry && data.signalType && (
               <div className="mt-2 pt-2 border-t border-gray-800">
                 <div className="text-gray-500 font-bold text-[10px] uppercase flex items-center gap-1">
                   <AlertTriangle size={10} /> {data.signalType} Signal (Already Long)
                 </div>
               </div>
            )}
  ...
  const CustomPrimeMarker = (props: any) => {
    ...
    if (payload.signalType) {
       const isPrime = payload.signalType === 'PRIME';
       return (
         <g>
           <circle cx={cx} cy={cy} r={3} fill={isPrime ? "#FACC15" : "#3B82F6"} opacity={0.6} />
  ...
  // New Pocket Pivot data handling
  const isPocketPivot = displayStock.technicalData.isPocketPivot;
  const instAction = rvol > 1.2 ? 'Accumulation' : rvol < 0.8 ? 'Distribution' : 'Neutral';
      ...
      <div className="relative bg-gray-900 border border-gray-700 w-full max-w-7xl max-h-[95vh] overflow-y-auto rounded-2xl shadow-2xl flex flex-col custom-scrollbar">
        <div className="sticky top-0 z-10 bg-gray-900/95 border-b border-gray-800 p-4 sm:p-6 flex justify-between items-start backdrop-blur">
          ...
          <div className="flex flex-col items-end gap-2 sm:flex-row-reverse sm:items-center">
            <button onClick={onClose} className="p-1.5 hover:bg-gray-800 rounded-full transition-colors text-gray-400 hover:text-white"><X size={24} /></button>
            <div className="flex gap-2">
                <button 
                    onClick={handleWatchlistClick}
                    className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-all shadow-lg font-bold text-sm border ${isWatched ? 'bg-blue-600 border-blue-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400 hover:text-white hover:bg-gray-700'}`}
                    title={isWatched ? "Remove from Watchlist" : "Add to Watchlist"}
                >
                    {isWatched ? <Check size={18} strokeWidth={3} /> : <div className="w-4 h-4 border-2 border-gray-400 rounded-sm"></div>}
                    <span className="hidden sm:inline">{isWatched ? 'Tracked' : 'Watchlist'}</span>
                </button>
                <button onClick={() => setShowAlertModal(true)} className="flex items-center justify-center p-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors shadow-lg shadow-blue-900/20" title="Set Alert"><Bell size={20} /><span className="hidden sm:inline ml-2 text-sm font-semibold">Set Alert</span></button>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-2 space-y-6">
            <div className="bg-gray-850 rounded-xl p-4 border border-gray-800 relative">
              ...
              <div className={`h-[300px] sm:h-[400px] w-full min-w-0 min-h-0 ${fibMode || riskMode ? 'cursor-crosshair' : ''} relative`}>
                 ...
                 {hasHistory && !showBacktest && isChartReady && (
                    <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                      <AreaChart data={chartData} onClick={handleChartClick} margin={{ right: 40 }} >
                        ...
                        {detectedPattern && (<ReferenceLine y={displayStock.technicalData.resistanceLevel} stroke="none" strokeWidth={1} strokeDasharray={2} label={{ position: 'insideTopLeft', value: `⚑ ${detectedPattern}`, fill: patternColor, fontSize: 12, fontWeight: 'bold', dy: -10 }} />)}
                        {renderFibLines()}
                 ...
                 {hasHistory && showBacktest && displayStock.backtest && displayStock.backtest.equityCurve && isChartReady && (
                     <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              ...
              {showBacktest && displayStock.backtest && (
                  <>
                    <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-4 bg-gray-900 rounded-lg p-4 border border-purple-500/20">
                        ...
                        <div><p className="text-[10px] text-gray-500 uppercase">Win Rate</p><p className={`${(displayStock.backtest.winRate ?? 0) > 50 ? 'text-green-400' : 'text-red-400'} font-mono font-bold`}>{displayStock.backtest.winRate?.toFixed(1) ?? 0}%</p></div>
                        <div><p className="text-[10px] text-gray-500 uppercase">Est. Return (252D)</p><p className={`${(displayStock.backtest.totalReturn ?? 0) > 0 ? 'text-green-400' : 'text-red-400'} font-mono font-bold`}>{displayStock.backtest.totalReturn > 0 ? '+' : ''}{displayStock.backtest.totalReturn?.toFixed(1) ?? 0}%</p></div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3">
                        <div className="bg-yellow-900/10 border border-yellow-500/20 rounded p-2 flex justify-between items-center">
                            <div>
                                <span className="text-[10px] font-bold text-yellow-500 uppercase flex items-center gap-1"><Target size={10} /> Prime Signal</span>
                                <span className="text-xs text-gray-400 block">{splitStats.primeWins}/{splitStats.primeTotal} Trades</span>
                            </div>
                            <div className="text-right">
                                <span className={`text-sm font-black font-mono ${splitStats.primeRate >= 50 ? 'text-green-400' : 'text-red-400'}`}>{splitStats.primeRate.toFixed(0)}%</span>
                                <span className="text-[9px] text-gray-500 block uppercase">Win Rate</span>
                            </div>
                        </div>
                        <div className="bg-blue-900/10 border border-blue-500/20 rounded p-2 flex justify-between items-center">
                            <div>
                                <span className="text-[10px] font-bold text-blue-500 uppercase flex items-center gap-1"><ArrowUpRight size={10} /> Trend Signal</span>
                                <span className="text-xs text-gray-400 block">{splitStats.trendWins}/{splitStats.trendTotal} Trades</span>
                            </div>
                            <div className="text-right">
                                <span className={`text-sm font-black font-mono ${splitStats.trendRate >= 50 ? 'text-green-400' : 'text-red-400'}`}>{splitStats.trendRate.toFixed(0)}%</span>
                                <span className="text-[9px] text-gray-500 block uppercase">Win Rate</span>
                            </div>
                        </div>
                    </div>
            ...
            {/* Institutional Footprint Section */}
            <div className="bg-gray-850 rounded-xl border border-gray-700 overflow-hidden">
               <div className="bg-gray-800/80 px-6 py-3 border-b border-gray-700 flex justify-between items-center">
                   <h3 className="font-bold text-white flex items-center gap-2"><Database className="text-blue-400" size={20} />Institutional Footprint</h3>
               </div>
               <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className={`border rounded-lg p-4 flex flex-col justify-center items-center text-center ${isPocketPivot ? 'bg-blue-900/10 border-blue-500/30 shadow-blue-900/10 shadow-lg' : 'bg-gray-800/30 border-gray-700'}`}>
                       <div className="flex items-center gap-2 mb-2">
                           <Zap size={18} className={isPocketPivot ? 'text-blue-400 fill-blue-400' : 'text-gray-500'} />
                           <span className={`text-sm font-bold uppercase tracking-wider ${isPocketPivot ? 'text-white' : 'text-gray-500'}`}>Pocket Pivot Signal</span>
                       </div>
                       <p className={`text-2xl font-black ${isPocketPivot ? 'text-blue-400' : 'text-gray-600'}`}>{isPocketPivot ? 'ACTIVE' : 'INACTIVE'}</p>
                       <p className="text-[10px] text-gray-500 mt-2">Institutional buying detected relative to recent supply.</p>
                   </div>
                   
                   <div className="space-y-4">
                       <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                           <span className="text-xs text-gray-400 uppercase font-bold">Inst. Action State</span>
                           <span className={`text-sm font-bold px-2 py-0.5 rounded uppercase ${instAction === 'Accumulation' ? 'bg-green-500/20 text-green-400' : instAction === 'Distribution' ? 'bg-red-500/20 text-red-400' : 'bg-gray-700 text-gray-400'}`}>
                               {instAction}
                           </span>
                       </div>
                       <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                           <span className="text-xs text-gray-400 uppercase font-bold">Vol Signature</span>
                           <div className="text-right">
                               <span className="text-white font-mono font-bold block">{formatVolumeCompact(displayStock.technicalData.lastVolume)}</span>
                               <span className="text-[10px] text-gray-500">{(rvol * 100).toFixed(0)}% of Average</span>
                           </div>
                       </div>
                       <div className="text-[10px] text-gray-500 italic">
                           Pocket Pivots occur when up-volume exceeds the highest down-volume of the prior 10 days.
                       </div>
                   </div>
               </div>
            </div>
                    ...
                    {hasPosition ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="text-xs text-gray-500 mb-1 block">Avg Entry Price</label><div className="relative"><span className="absolute left-3 top-2 text-gray-500">$</span><input type="number" value={entryPrice} onChange={(e) => setEntryPrice(e.target.value)} placeholder="0.00" className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 pl-6 text-white focus:outline-none focus:border-blue-500"/></div></div>
                                    <div><label className="text-xs text-gray-500 mb-1 block">Shares</label><input type="number" value={shares} onChange={(e) => setShares(e.target.value)} placeholder="0" className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500"/></div>
                                </div>
                                <button onClick={handleSavePosition} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white py-2 rounded-lg transition-colors text-sm font-semibold"><Save size={16} /> Update Position</button>
                                {tradeAdvice && (<div className="mt-4 p-3 bg-gray-800/50 rounded-lg flex justify-between items-center border border-gray-700"><div><p className="text-xs text-gray-400">Current P&L</p><p className={`text-lg font-bold ${tradeAdvice.pnlPercentage >= 0 ? 'text-green-400' : 'text-red-400'}`}>{tradeAdvice.pnlPercentage > 0 ? '+' : ''}{tradeAdvice.pnlPercentage}%</p></div><div className="text-right"><p className="text-xs text-gray-400">Value</p><p className="text-lg font-bold text-white">${(displayStock.currentPrice * (parseFloat(shares) || 0)).toLocaleString()}</p></div></div>)}
                            </div>
                            <div className="bg-gray-800/30 border border-gray-700 rounded-xl p-5 relative overflow-hidden">
                                {tradeAdvice ? (
                                    <>
                                        <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
                                        <h4 className="text-sm font-bold text-gray-300 uppercase tracking-wider mb-3 flex items-center gap-2">AI Trade Advisor</h4>
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className={`px-3 py-1.5 rounded-lg font-bold text-sm uppercase ${tradeAdvice.action === 'BUY MORE' ? 'bg-green-500/20 text-green-400' : tradeAdvice.action === 'SELL / TRIM' ? 'bg-orange-500/20 text-orange-400' : tradeAdvice.action === 'CUT LOSS' ? 'bg-red-500/20 text-red-400' : tradeAdvice.action === 'TAKE PROFIT' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-500/20 text-blue-400'}`}>{tradeAdvice.action}</div>
                                            <span className="text-xs text-gray-500">Based on score {displayStock.totalScore} & entry</span>
                                        </div>
                                        <p className="text-sm text-gray-300 italic mb-4">"{tradeAdvice.reason}"</p>
                                        <div className="grid grid-cols-2 gap-4 border-t border-gray-700/50 pt-3">
                                            <div><p className="text-[10px] text-gray-500 uppercase">Suggested Stop</p><p className="text-red-400 font-mono font-bold">${tradeAdvice.suggestedStop}</p></div>
                                            <div><p className="text-[10px] text-gray-500 uppercase">Next Target</p><p className="text-green-400 font-mono font-bold">${tradeAdvice.suggestedTarget}</p></div>
                                        </div>
                                    </>
                                ) : (
                        ...
                        <div className="p-2">
                            <div className="bg-blue-900/10 border border-blue-500/20 rounded-xl p-5">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="text-sm font-bold text-blue-400 flex items-center gap-2"><Calculator size={16}/> Position Sizing Calculator</h4>
                                    <div className="text-[10px] text-gray-500 bg-gray-800 px-2 py-1 rounded border border-gray-700">R = P × r / D</div>
                                </div>
                                
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        <label className="text-[10px] text-gray-500 font-bold uppercase mb-1 block">Portfolio Capital ($)</label>
                                        <div className="relative">
                                            ...
                                            <input 
                                                ...
                                                className="w-full bg-gray-800 border border-gray-700 rounded-lg py-2 pl-8 pr-3 text-white font-mono text-sm focus:border-blue-500 outline-none"
                                            />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="text-[10px] text-gray-500 font-bold uppercase mb-1 block">Risk Per Trade (%)</label>
                                        <div className="grid grid-cols-3 gap-2">
                                            {[0.5, 1.0, 2.0].map(val => (
                                                <button 
                                                    key={val}
                                                    onClick={() => setCalcRiskPct(val)}
                                                    className={`py-2 rounded-lg text-xs font-bold transition-all border ${calcRiskPct === val ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-800 text-gray-400 border-gray-700 hover:bg-gray-700'}`}
                                                >
                                                    {val}%
                                                </button>
                                            ))}
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-6 pt-4 border-t border-blue-500/10">
                                    <div>
                                        <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Max Risk ($)</p>
                                        <p className="text-white font-mono font-bold">${calcData.maxRiskDollars.toFixed(0)}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Stop Dist (2.2 ATR)</p>
                                        <p className="text-red-400 font-mono font-bold">${calcData.stopDistance.toFixed(2)}</p>
                                        <p className="text-[9px] text-gray-600">Stop Price: ${calcData.suggestedStopPrice.toFixed(2)}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Alpha Factor</p>
                                        <p className={`font-mono font-bold ${calcData.factorColor}`}>{calcData.factorLabel}</p>
                                    </div>
                                </div>

                                <div className="flex items-center gap-4 bg-gray-900 border border-gray-700 p-4 rounded-xl">
                                    <div className="flex-1">
                                        <p className="text-[10px] text-gray-500 font-bold uppercase">Recommended Size</p>
                                        <p className="text-2xl font-black text-white">{calcData.finalShares} <span className="text-sm font-normal text-gray-500">Shares</span></p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] text-gray-500 font-bold uppercase">Capital Required</p>
                                        <p className="text-xl font-mono text-blue-400 font-bold">${calcData.capitalRequired.toLocaleString()}</p>
                                    </div>
                                    {calcData.finalShares > 0 ? (
                                        <button 
                                            onClick={applyCalculatedPosition}
                                            className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-sm transition-colors shadow-lg"
                                        >
                                            Apply
                                        </button>
                                    ) : (
                                        <div className="bg-gray-800 text-gray-500 px-4 py-2 rounded-lg font-bold text-xs border border-gray-700 cursor-not-allowed">
                                            No Trade
                                        </div>
                                    )}
                                </div>
                                <div className="mt-3 flex items-start gap-2">
                                    ...
                                    <p className="text-[10px] text-gray-500 leading-tight">
                                        Calculation: Base quantity determined by risking {calcRiskPct}% of capital if price hits stop (-2.2 ATR). 
                                        Final quantity is adjusted by Alpha Score ({calcData.alphaFactor * 100}% exposure).
                                    </p>
                                </div>
                            </div>
                        </div>
          ...
          <div className="flex flex-col gap-6 h-full">
            <div className="bg-gray-850 p-6 rounded-xl border border-gray-800 text-center shadow-lg relative overflow-hidden flex-shrink-0">
                <div className="absolute top-2 right-2 opacity-20">{isTrendUp ? <TrendingUp size={80} className="text-green-500" /> : <TrendingDown size={80} className="text-red-500" />}</div>
                ...
                <div className="relative inline-flex items-center justify-center">
                    <svg viewBox="0 0 160 160" className="w-32 h-32 transform -rotate-90">
                        <circle className="text-gray-800" strokeWidth="10" stroke="currentColor" fill="transparent" r="70" cx="80" cy="80"/>
                        <circle 
                        ...
                        <div key={idx} title={ind.criteria} className="flex items-start gap-3 p-1.5 hover:bg-gray-800/40 rounded-lg transition-colors cursor-help">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-center mb-1">
                                    ...
                                    <div className="flex items-center gap-2"><span className="text-[10px] text-gray-600 bg-gray-800 px-1 rounded">{(ind.weight * 100).toFixed(0)}%</span><span className={`text-sm font-bold ${ind.score >= 8 ? 'text-green-400' : ind.score <= 3 ? 'text-red-400' : 'text-yellow-400'}`}>{ind.score}</span></div>
                                </div>
                                <p className="text-xs text-gray-500 leading-tight">{ind.description}</p>
                                <div className="mt-1 text-[10px] text-gray-500">Val: <span className="text-gray-300 font-mono">{ind.value}</span></div>
                            </div>
                        </div>
                    ))}
                </div>
